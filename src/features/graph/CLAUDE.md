...
source/target 정규화(swap): ① main(프로젝트) 노드 → 항상 source, ② 단독 노드(엣지 0개) ↔ 그래프 연결이면 그래프 쪽 → source.
